 ...
 */
class mrBoxieAdminThemeConfiguration extends sfThemeConfiguration
{
    protected
    $theme = 'boxie_admin';
    ...
    public function filesToCopy()
    {
        ...
        if (sfConfig::get('app_boxie_admin_include_flashes', true))
        {
            $files['APP_DIR/templates/_flashes.php'] = 'THEME_DIR/templates/_flashes.php';
        }

        if (sfConfig::get('app_boxie_admin_include_assets', true))
        {
...
EOF
  ...
  class: mrBoxieAdminRouteCollection
  options:
    model:                %s
    module:               %s
    prefix_path:          /%s
